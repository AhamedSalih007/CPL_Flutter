...
class ApiManagerImpl extends ApiManager with SerializerMixin {
  ...
  Future<ApiResponse<T>> requestAsync<T extends ToJson>({
    ...
    return _tryApiRequest<T>(
      () async {
        ...
        try {
          var data = serializeResponseData<T>(
              response.data.runtimeType == String
                  ? jsonDecode(response.data)
                  : response.data as Map<String, dynamic>?);
          return data;
        } catch (e) {
          return serializeResponseData<T>(
    ...
    return _tryApiRequest<List<T>>(
      () async {
        try {
          ...
          return serializeResponseDataList<T>(
              (response.data.runtimeType == String
                  ? jsonDecode(response.data)
                  : response.data) as List?);
        } catch (e) {
          return serializeResponseDataList<T>([] as List?);
        }
      },
    );
  }
  ...
  Future<ApiResponse<T>> _tryApiRequest<T>(
    ...
    try {
      if (_ref.read(connectivityServiceProvider).isConnected == true) {
